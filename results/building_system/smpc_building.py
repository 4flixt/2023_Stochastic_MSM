...
x0 = np.array([26, 20, 20, 20, 10]).reshape(-1,1)
...
ss_mpc_settings = smpc.base.SMPCSettings(
    prob_chance_cons=.999,
    with_cov=True,
)

ss_mpc = smpc.StateSpaceSMPC(ssm, ss_mpc_settings)

ss_mpc.set_objective(
    Q    = 0*np.eye(msm.n_y),
    delR = 5*np.eye(msm.n_u),
    R    = 10*np.eye(msm.n_u),
)
...
y = ss_mpc._y_stage
ss_mpc.set_chance_cons(expr =  y, ub = T_ub)
ss_mpc.set_chance_cons(expr = -y, ub = -T_lb)
ss_mpc.set_chance_cons(expr =  -y[0]+y[1], ub = -1)

ss_mpc.setup()

ss_mpc.lb_opt_x['u_pred',:] = np.array([-6, -6, -6, -6, 10])
ss_mpc.ub_opt_x['u_pred',:] = np.array([6, 6, 6, 6, 10])
...
meas_lines = ax[0].plot(t_pred, ss_mpc.res_y_pred)

ax[0].set_prop_cycle(None)
for k in range(msm.n_y):
    ax[0].fill_between(t_pred, 
                       ss_mpc.res_y_pred[:,k]+ss_mpc.cp*ss_mpc.res_y_std[:,k], 
                       ss_mpc.res_y_pred[:,k]-ss_mpc.cp*ss_mpc.res_y_std[:,k], 
                       alpha=.2)
    
ax[0].set_prop_cycle(None)
ax[0].plot(t_past, ss_mpc.res_y_past, '-x')
ax[0].axhline(T_lb, color='k', linestyle='--')
ax[0].axvline(0, color='k', linestyle='-')
ax[0].text(-2, 18.2, '$\\leftarrow$ past')
ax[0].text(.5, 18.2, 'pred. $\\rightarrow$')

input_lines = ax[1].step(t_pred[:-1],ss_mpc.res_u_pred[:-1,:4], where='post')
ax[1].set_prop_cycle(None)
ax[1].step(t_past,ss_mpc.res_u_past[:,], '-x', where='post')
...
ss_mpc.read_from(sys)
sys.simulate(ss_mpc, 50)
# %%
...
ax.plot(sys.y[:,1], sys.y[:,0], label='y(t)')
ax.plot(ss_mpc.res_y_pred[:,1], ss_mpc.res_y_pred[:,0], label='y_pred(t)')
...
cov0 = ss_mpc.opt_aux_num['Sigma_y_pred'][:2,:2].full()
covN = ss_mpc.opt_aux_num['Sigma_y_pred'][-4:-2,-4:-2].full()
e1=helper.plot_cov_as_ellipse(ss_mpc.res_y_pred[0,1], ss_mpc.res_y_pred[0,0], cov0, ax=ax, n_std=ss_mpc.cp, edgecolor=colors[0], facecolor=colors[0], alpha=0.2)
e2=helper.plot_cov_as_ellipse(ss_mpc.res_y_pred[-1,1], ss_mpc.res_y_pred[-1,0], covN, ax=ax, n_std=ss_mpc.cp, edgecolor=colors[1], facecolor=colors[1], alpha=0.2)
e1.set_label('covariance at t=0')
e2.set_label('covariance at t=N')
ax.legend()
# %%
